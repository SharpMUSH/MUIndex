...
if (connectionString is not null)
{
    ...
    builder.Services.AddMuiCrawler(connectionString, configure =>
    {
        ...
        // Whether this replica crawls at all, and what it knows on day one. The advisory lock
        // already guarantees one crawler across N replicas, so MUI_CRAWL_ENABLED=false is a
        // deliberate choice about where the crawl runs rather than a safety net.
        configure.Apply(builder.Configuration);
    });
}
else
{

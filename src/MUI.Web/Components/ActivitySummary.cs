...
public static class ActivitySummary
{
    ...
    /// <summary>One line per day, for the "read as text" disclosure and for plain mode.</summary>
    public static IReadOnlyList<string> PerDay(IReadOnlyList<ActivityCell> cells)
    {
        ...
        foreach (var day in cells.Select(c => c.DayOfWeek).Distinct().Order())
        {
            ...
            if (counted.Count > 0)
            {
                ...
                clauses.Add(peak.Count == 0
                    ? "measured at zero all day"
                    : $"peak {peak.Count} at {peak.Hour:00}:00");

                var quiet = LongestRun(forDay, c => c.IsCounted && c.Count == 0);
                if (quiet is { } q)
                {
                    clauses.Add($"nobody on {q.From:00}:00–{q.To:00}:59");
                }
            }
            else
            {
            ...
            if (unmeasurable > 0)
            {
                clauses.Add($"{unmeasurable} {Hours(unmeasurable)} probed but uncountable");
            }

            lines.Add($"{ShortDayName(day)} — {string.Join(", ", clauses)}");
        }

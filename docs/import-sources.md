...
It is a scrape: one index fetch plus one fetch per world, with no dump and no API. It sat behind
`ContactedMaintainer` until its maintainer was approached and the run authorised; the flag is now
true **for this source only**. Everything else §7.6 asks for is enforced in code rather than
...
**This is what the contacted-maintainer gate is now for.** `MudVerseSource` is written and tested;
